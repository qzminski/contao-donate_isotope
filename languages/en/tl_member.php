<?php

/**
 * isotope_donate extension for Contao Open Source CMS
 *
 * Copyright (C) 2013 Kamil Kuzminski
 *
 * @package isotope_donate
 * @author  Kamil Kuzminski <kamil.kuzminski@codefog.pl>
 * @license LGPL
 */


/**
 * Fields
 */
$GLOBALS['TL_LANG']['tl_member']['donate_list'] = array('Display on the donators page', 'Here you can choose whether the member will be displayed on the donator list.');


/**
 * Reference
 */
$GLOBALS['TL_LANG']['tl_member']['donate_list']['hide']    = 'Do not display';
$GLOBALS['TL_LANG']['tl_member']['donate_list']['name']    = 'Display name';
$GLOBALS['TL_LANG']['tl_member']['donate_list']['company'] = 'Display company';


/**
 * Legends
 */
$GLOBALS['TL_LANG']['tl_member']['donate_legend'] = 'Donator settings';
