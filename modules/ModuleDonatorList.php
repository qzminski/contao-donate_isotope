...
class ModuleDonatorList extends \Module
{
	...
	{
		$objMembers = $this->Database->execute("SELECT DISTINCT tl_member.* FROM tl_member INNER JOIN tl_donation ON tl_member.id=tl_donation.member WHERE tl_member.donate_list!='' ORDER BY tl_donation.datePaid");
